...
class Ball:
    def __init__ (self,canvas,color,paddle,paddle1):
        ...
        self.canvas.move(self.id,232,200)
        starts = [-3,3]
...
class Paddle:
    def __init__ (self,canvas,color):
        self.canvas = canvas
        self.id = canvas.create_rectangle(0,150,30,250,fill=color) #(x1,y1) the coordinates of the top left corner and (x2, y2) those of the bottom right corner
        self.y = 0
        self.canvas_height = self.canvas.winfo_height()
...
ball = Ball(canvas,"blue",paddle,paddle1)
...
while 1:
    ...
    if counter == 3:
        ...
        canvas.create_text(250,200,text = "Score:" + str(counter) + "to" + str(counter1),font=32,fill="red" )
    if counter == 3:
        ball.x == 0
        ball.y == 0
        paddle.y == 0
        paddle1.y == 0
        canvas.create_text(250,200,text = "Congrats Player 2ad You win", font=32,fill="red")
        canvas.create_text(250,200,text = "Score:" + str(counter) + "to" + str(counter1),font=32,fill="red" )

    tk.update_idletasks()
    tk.update()
    time.sleep(0.01)
    if counter==3 or counter1==3:
        time.sleep(100000)